...
### ✅ Completed (v3.1.0 - Code Review Fixes)
- [x] P0 安全修复：JWT HttpOnly Cookie、MCP 连接泄漏修复、SSE 重连修复
- [x] P1 性能优化：N+1 查询、LangGraph Node 签名统一、重试机制、工具异步化
- [x] P2 类型优化：TypeScript any 清理（router.tsx）
- [x] P2 缓存优化：MCP 工具 TTL 缓存 (5分钟)
- [x] 新增 Hooks：useOptimisticUpdate、useSuspenseQuery（可选使用）
