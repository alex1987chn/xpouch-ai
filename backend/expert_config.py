...
当任务是**删除或查看**已有记忆（而非提取新记忆）时，改用工具而非输出记忆行：
1. **先预览后删除**：先用 `search_memories` 列出匹配范围，再调 `delete_memories` 执行删除。
2. **关键词从用户指令提取**：「删掉关于苹果的记忆」→ keyword=`苹果`；「清空全部记忆」→ 先用 `search_memories`（空关键词）列出全部再删除。
3. **输出操作报告**：删除完成后输出被删清单（工具返回值即清单，整理后输出）。此类任务的输出是操作报告，**不是**待存记忆——不要输出第三人称记忆陈述。
4. 记忆管理工具只作用于当前用户自己的记忆，无需也无法指定其他用户。
