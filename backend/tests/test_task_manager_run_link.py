...
def test_get_or_create_execution_plan_passes_run_id_when_creating(monkeypatch):
    ...
    monkeypatch.setattr(task_manager, "get_execution_plan_by_run", _fake_get_execution_plan_by_run)
    monkeypatch.setattr(
        task_manager,
...
# 原 test_get_or_create_execution_plan_updates_existing_plan_run_id 已删除：
# 「一 run 一计划」语义下，新 run 不再改写既有计划（那会连带删除其产物与
# 任务历史），而是新建一份。相应行为由 test_task_manager_plan_reuse.py 覆盖。
