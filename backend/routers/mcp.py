...
async def test_mcp_connection(
    sse_url: str, transport: str = "sse", timeout: int = 10
) -> tuple[bool, str]:
    """
    测试 MCP 服务器连接
...
async def create_mcp_server(
    ...
    is_connected, error_msg = await test_mcp_connection(server_data.sse_url, transport=transport)
    ...
    if existing:
        raise ValidationError(
            message="该 MCP 服务器已存在", details={"sse_url": server_data.sse_url}
        )
...
async def get_mcp_server_tools(
    ...
    try:
        ...
        raise ValidationError("获取工具列表超时 (10秒)") from e
    except Exception as e:
        raise ValidationError(f"获取工具列表失败: {str(e)}") from e
