...
- **专家执行轮消息顺序两态不一致（实时反序 / 刷新变序）**：实时流的聚合正文写进**发送时预建**的占位消息（还把它交给后端当聚合落库 id），专家消息后到插在它后面——综述显示在专家上方；刷新后按库序（专家在前、思考挂聚合消息）——两态都不等于因果序。修法（消息表行序=因果序）：①commander 规划落库时插入**思考载体消息**（content 恒空，排在专家之前；幂等按 run_id，驳回修订不重复插），`plan.created` 事件携带其 id，前端把占位消息**原位改写**成它——实时流与刷新回放共用同一条消息；②聚合消息 id 与前端解耦：run 创建时生成 `state.aggregate_message_id` 贯穿全图（aggregator 落库、流式转换层、事件三者同源），聚合 delta 按事件自身 id 在**尾部**创建消息（SSE 层透传事件给 onChunk + RAF 批处理层新增 retarget 按目标路由）——聚合正文永远排在所有专家之后；③刷新回放的思考重建改挂该 run 的载体行（老会话无载体行时回退挂最后一条助手消息）；④resume 的 message_id 贯通链整体退役（ResumeRequest 字段、recovery/stream 两层传递、占位预建、done 时的占位步骤迁移块——其服务的中断恢复场景已由载体行落库根治），LLM 上下文构建跳过空正文载体行。简单模式（direct_reply）不受影响。E2E 三层验证（实时 DOM 序 / 刷新回放序 / DB 行序）同一因果序：思考 → 专家们 → 聚合
