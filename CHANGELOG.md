...
- **产物标题落库改为产出真实标题**（此前写死「{专家}结果」，画廊/产物卡/预览弹窗/下载文件名全是通用名，无法区分同类产物）：generic 落库处提取——优先产出里第一个 markdown 标题行；无标题时首行仅当短（≤40 字符）且无句读才采用（模型过渡句如 "I have sufficient information…" 是完整长句，必须兜底——E2E 实抓的规则漏洞）；标题的单一真相源在 artifact 表，展示层不重复推导。画廊卡片已有 line-clamp-2 截断（长标题省略），前端零改动。存量产物保持原样不回填
- **专家卡工具明细可展开 + 任务描述展开 + 产物横条真实标题**（对照 Manus / ChatGPT Deep Research / Devin 的过程可回看模式——行业"过程流式"= 动作流而非草稿正文流，故不做中间轮草稿流式）：①完成态工具汇总行（N 次 · 总耗时）点击展开逐次明细（工具名 / MCP 标 / 耗时 / 成败色点）——后端从账本聚合**逐次明细快照**随消息落库（extra_data.tool_calls，截 50 条）并经 task.completed 事件下发（事件载荷=库内终态）；②执行中的工具活动从"只显示当前一条"升级为**实时追加的活动序列**（逐次累积进消息 metadata，渲染最近 3 条 + 更早计数）；③任务描述超长时（>48 字符）一行截断 + chevron 展开/收起（默认紧凑，展开看完整任务要求）；④产物横条从通用文案"查看本步骤产出"改为显示**产出首行标题**（通常是报告标题，如"Bun 与 Deno 核心区别总结"）——与完成摘要同源，有横条时不再重复渲染摘要行；⑤专家卡加左侧 2px 专家识别色条（克制的容器感，扁平无底色），同轮连续专家卡间距收紧成组（组内 12px / 组间 32px），强化过程块的视觉归属

### 修复

- **消息卡产物横条标题与画廊不一致（summary 未清洗，用户实报）**：横条此前显示产出首行**原文**（summary 无清洗规则），碰到模型过渡句/长句整段糊上去很乱；画廊显示的 artifact.title 却是清洗过的（markdown 标题优先→短无句读→兜底）。两处本该是同一个东西却有两套生成逻辑——收敛为**单一提取规则**（`utils.title_extract`）：summary 生成直接取 artifact.title（generic 构造产物时已提取），task.completed 事件携带权威标题（前端实时覆盖不再用 output 首行兜底猜）。迁移 20260923_000600 把存量消息 summary 同步为其产物 title（实测对齐，含用户会话的上海攻略轮）。generic 本地函数挪 utils 共用，无兼容层
...
- **专家执行轮消息顺序两态不一致（实时反序 / 刷新变序）**：实时流的聚合正文写进**发送时预建**的占位消息（还把它交给后端当聚合落库 id），专家消息后到插在它后面——综述显示在专家上方；刷新后按库序（专家在前、思考挂聚合消息）——两态都不等于因果序。修法（消息表行序=因果序）：①commander 规划落库时插入**思考载体消息**（content 恒空，排在专家之前；幂等按 run_id），`plan.created` 事件携带其 id，前端把占位消息**原位改写**成它——实时流与刷新回放共用同一条消息；②聚合消息 id 与前端解耦：run 创建时生成 `state.aggregate_message_id` 贯穿全图（aggregator 落库、流式转换层、事件三者同源），聚合 delta 按事件自身 id 在**尾部**创建消息（SSE 层透传事件给 onChunk + RAF 批处理层新增 retarget 按目标路由）——聚合正文永远排在所有专家之后；③刷新回放的思考重建改挂该 run 的载体行（老会话无载体行时回退挂最后一条助手消息）；④resume 的 message_id 贯通链整体退役（ResumeRequest 字段、recovery/stream 两层传递、占位预建、done 时的占位步骤迁移块——其服务的中断恢复场景已由载体行落库根治），LLM 上下文构建跳过空正文载体行。简单模式（direct_reply）不受影响。E2E 三层验证（实时 DOM 序 / 刷新回放序 / DB 行序）同一因果序：思考 → 专家们 → 聚合
- **思考过程卡在聚合回复出来后消失 + 刷新后不重建（三处叠加）**：①中断-恢复链路的思考步骤挂在流①的占位消息上（当时的最后一条 AI 消息），正文落在流②的新消息（resume 生成新 message_id）——done 时把占位消息的步骤迁到正文消息并移除空占位，否则历史 refetch 用服务端列表整体替换后步骤蒸发；②会话恢复替换消息时按 id 把本地已有 thinking 并回（防 refetch 二次冲刷）；③词汇收敛漏跟：useSessionRestore 读已退役的 `message.timestamp`（服务端早已发 `created_at`），守卫恒为"消息时间=0<run 开始"直接跳过——**账本重建从未执行过**，刷新后思考卡从不出现（同 task_description 漏跟同族）。修后与官方做法一致：执行中实时展示，完成后折叠保留（专家步骤+工具调用明细可回看），刷新后从事件账本重建

### 变更

- **专家执行产出改为独立助手消息流（消息表=执行状态一等真相源）**：此前专家执行过程全部折叠在 thinking 卡里（嵌套分组 + 任务行 + 产物行 + 工具行），产物与结论混在中间态 UI，用户上滚看不到"谁做了什么"。现改为：thinking 卡只留路由判断与规划两个里程碑；每个专家任务开始时即在消息表插入 running 态骨架消息（`extra_data.message_kind='expert_result'`），完成/失败原位更新为终态（摘要/产物引用/工具统计快照/耗时），前端实时流按 message_id 追加与覆盖同一条消息——**执行中刷新页面也能从库恢复现场**，刷新后产物横条、步骤序号、工具统计全部回放（经真实 LLM E2E 三态验证）。消息卡形态：专家识别色署名 + 步骤 i/N + 任务描述 + 完成摘要 + 工具区（执行中实时转圈、完成后聚合统计）+ 产物横条（点击拉取详情弹预览）。同时修掉 task.completed 事件先于落库发射的时序病（挪进保存协程，与 artifact.generated 同纪律：事件载荷即消息终态）。**旧机制删净**：thinking 卡的 execution 步骤渲染/专家分组/产物卡/工具明细行、thinkingGroups 分组逻辑（退化为平铺）、thinkingStepsFromTimeline 的 task/tool 账本重建分支、ThinkingStep 的 toolActivity/toolHistory 字段全部退役；工具明细真相源仍是 runevent 账本（时间线页可查），消息上的 tool_stats 是完成时刻的聚合快照。对照结论：DeepSeek/Kimi/Claude/ChatGPT 把思考收进单块，Manus 是过程消息独立成条的成功先例——多专家 + 产物形态下独立成条更合理
...
- **产物画布不自动刷新（要手动刷新整页才出卡）**：`artifact.generated` 事件在产物落库**之前**发射（保存是后台线程异步的，事件紧跟 spawn 立即发出），前端收到事件去拉产物列表恰在落库提交前扑空，之后无任何补偿刷新。修法：事件改由保存协程在落库成功后发射——协议语义收敛为「收到即可查」；保存失败不发（产物不存在就不宣称已生成）。流结束的兜底刷新同样可能撞上最后产物的落库窗口，一并根治
- **运行统计页整页「加载失败」（一条坏行炸整个列表）**：run 出生时 mode 写占位值 `"router"`（路由决策后更新 simple/complex），**决策前终止的 run**（立即停止/断流/重启）永远停在该值，被统计/详情接口的 Literal 契约拒收——一条即炸整个响应。修法：没决策如实存 NULL（迁移 20260923_000100，存量 router 值清洗），契约放宽为可空 str 并对未迁移库存量值宽容；前端空值显示 —
- **运行时间线页三处显示问题**：hover 高亮条文案贴顶（行距与 hover 背景同层）；文字与 32px 图标顶对齐错位 6px；右侧执行计划步骤只剩「✓ completed」没有任务描述——词汇收敛改列名（task_description→description）时前端三处消费点漏跟，读 undefined 渲染空白。计划卡长描述改单行截断（悬停看全文）
- **内置专家教材七项缺陷（审计根治，教材单一真相源 = expert_config）**：commander 代码种子仍教已删除的 `priority` 字段（词汇收敛只清洗了 DB）；router/aggregator 无代码种子——空库缺失后走 constants 静态兜底，而静态版与 DB 版脑裂（无占位符注入点，路由失去时间/记忆上下文）；search 教材的 `{current_time}` 是死占位符（generic worker 从不替换）；story_writer 种子指定无权限的 kimi-k2.5；空库灌种的「内置」专家全部标成可删除（is_dynamic 走了模型默认）。迁移 20260923_000200 对全部内置专家 INSERT IF NOT EXISTS（**迁移即种子**：只补 router/aggregator 会让表非空、启动自举跳过、其余专家永远缺失）+ 教材下发 + 内置标记归位（用户自建的动态专家不碰）
- **记忆助理教材与消费端错配**：教材教 JSON 数组（category/validity），但记忆存储是纯 content 文本 + 向量检索——结构字段无人承接，空数组 `[]` 被整段存成垃圾记忆。教材改为「一行一条记忆、无可记只输出：无」，消费端逐行入库、「无」跳过、回复如实
- **搜索专家教材与执行框架指令打架**：One-Shot 禁令（严禁再次调用工具）与框架头部注入的工具循环/防偷懒协议互相矛盾，改为一致化表述（单轮为主、结果明显不足允许追加一次）

### 变更（2026-09-23 批次）

- **审计日志分页**：原为一次性拉前 100 条直出、无总数、第 101 条起不可见。改为 20 条/页 + 右下角翻页条（首页/末页禁用、翻页保留旧页不闪骨架屏、换搜索词回第 1 页），顶部计数改过滤后总数；接口响应加 `{items,total,limit,offset}` 分页包装（前端契约锚点同步）
- **小说家（story_writer）移出内置专家清单**：题材型创作专家不属系统内置（内置 12→11），需要者经管理台自建；现网已自建的动态行不受影响
...
- **删除 CustomAgent 全链路**：用户自定义智能体（个人 system_prompt 直连 LLM，不走 LangGraph）的前端入口在 v3.4.7 改版时已退役（创建页路由删除），后端随之悬空——没有任何路径能把 CustomAgent UUID 递到 `?agent_id=`。本批删除整条链路：后端 5 端点 + 服务 + 模型 + 聊天分支 + 枚举；前端 services/hooks/store/组件消费；数据库表 + 历史 custom 线程（2 条）+ 枚举收窄。历史会话显示名退化为 slug（数据已清）
...
- **许可改为标准 Apache License 2.0（去掉附加条款）**：此前是 "Apache 2.0 + 附加条款"（SaaS 限制 + 品牌保护），而附加条款恰恰让它不再是 Apache 2.0——项目变成 source-available，GitHub 也识别不出许可（显示 "Other"）。现 `LICENSE` 逐字采用 Apache 官方原文（经 difflib 与 apache.org 比对 0 差异），任何人可自由使用、修改、分发与商用（含作为托管服务对外提供）；署名要求集中到新增的 `NOTICE` 文件（Apache §4d）。README 双语的许可徽章与段落同步，贡献者条款留在 `CONTRIBUTING.md`
...
- **审批 UX（设计师反馈两则）**：审批弹窗头部/底部锁定（仅内容区滚动，长计划的批准按钮不再被顶出首屏）；审批行与弹窗头图标 AlertTriangle → ClipboardList（不再读作告警/错误，琥珀色「待裁决」语义保留）
- **CI：模型↔迁移漂移闸门转正**：`alembic check` 从「仅报告」改为阻塞；顺带修复 `migrations/env.py` 缺失的模型导入——此前 `target_metadata` 为空，autogenerate 会生成「删除所有表」的降级脚本
- **新增两条流式语义回归测试**（真图直驱服务层）：规划断连不杀 producer（断连后 run 照常到达审批点、审批卡可从持久帧补放）；wave 执行中协作取消一致性（终态正确、在途调用不悬空、重复取消幂等）
- 内部清扫：前后端死代码与过时注释、i18n 硬编码词条收编、删除只写不读的 PlanningSlice 本地副本、若干查询收窄与超时口径统一
...
- **HTML 产物预览顶部出现「index.html」字样（用户实测报出）**：模型常给产物起名
  ...
  审计外的产物下载/导出同样受益
- **专家多轮工具调用静默失效（重要）**：执行子图重入时未重新绑定工具——只有首轮有
  工具，第二轮起搜索/时间/MCP 全部失效且无报错。现每轮重入即重绑，仅工具失控熔断
  后才无工具收尾（含回归测试锁定）
- **流式输出切会话串台**：SSE 分发点补会话归属守卫——执行中途切换会话，旧流的增量
  帧不再写进新会话的消息列表
- **规划失败被伪装成空计划**：Commander 异常曾被吞成空任务列表继续走审批
  （用户看到「0 个任务的成功计划」），现显式标失败并下发错误事件
- **create_all 时代老库升级失败（部署向）**：schema 对齐迁移的索引改名全部守卫化
  （旧名存在 + 新名缺失 + 旧索引位于活表三条件；死表上的同名索引不得占用新名），
  外键与非空约束补存在性守卫。create_all 时代的老库（含生产）此前升级到此迁移必炸
- **专家配置加载失败拖死整份计划**：纳入任务级失败隔离（产出 failed 结果并写明原因，
  兄弟任务照常），不再中断整轮执行
- **聚合上下文去重**：上游成果全文只注入一次（此前同一产出可能重复进入下游 prompt）；
  失败任务在聚合输入中显式标注，不再静默消失
