...
- 问题: 使用 `AgentPreviewCard` 组件但未导入
- 修复: 添加 `AgentPreviewCard` 组件导入，移除重复的 `PreviewCard` 内联组件
- 影响: 修复了创建智能体页面的运行时错误

**Sidebar.tsx - 个人中心菜单无法打开**
- 问题: `SidebarSettingsMenu` 的 `isOpen` 硬编码为 `false`，导致菜单状态无法同步
- 修复: 添加 `isSettingsMenuOpen` 状态管理，连接 `SidebarUserSection` 和 `SidebarSettingsMenu`
- 影响: 修复了点击个人头像无法打开设置菜单的问题

**SidebarSettingsMenu.tsx - 缺少图标导入**
- 问题: 使用了 `Star`, `Plane`, `Crown` 图标但未导入
- 修复: 从 `lucide-react` 添加缺失的图标导入
- 影响: 修复了点击个人头像时的运行时错误

### 🔧 代码优化
- 移除 `Sidebar.tsx` 中未使用的 `isOnChat` 和 `location` 变量
- 消除所有 linter 警告
