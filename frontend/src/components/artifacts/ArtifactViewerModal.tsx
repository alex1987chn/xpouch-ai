...
export function ArtifactViewerModal({ artifactId, onClose, threadId }: ArtifactViewerModalProps) {
  ...
  const a11y = useDialogA11y<HTMLDivElement>(open, 'artifact-viewer-title')
  ...
  const handleDownload = () => {
    if (!detail?.content) return
    downloadBlob(detail.content, TYPE_EXT[detail.type] || 'txt')
  }
  ...
  const createdLabel = detail?.created_at
    ? formatDistanceToNow(new Date(parseISO(detail.created_at).getTime() - new Date().getTimezoneOffset() * 60_000), { addSuffix: true, locale })
    : ''
  const isMdAble = !!detail && MD_TYPES.has(detail.type)
  const canEdit = !!detail && EDITABLE_TYPES.has(detail.type)
      ...
      {createPortal(
        ...
        >
          <div
            ...
            className="flex h-[84vh] w-[min(920px,94vw)] flex-col overflow-hidden rounded-xl border border-border-default bg-surface-card shadow-theme-modal"
          >
            {/* 头部：类型 + 标题 + 动作组 */}
