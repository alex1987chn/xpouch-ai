...
export default function CodeArtifact({ content, language = 'text', className }: CodeArtifactProps) {
  ...
  useEffect(() => {
    ...
    const highlightedLines = lines.map(line =>
      keywords.length > 0 ? highlightKeywords(line, keywords) : line
    )

    containerRef.current.innerHTML = highlightedLines
      .map((line, idx) => `<div class="flex"><span class="w-10 text-right text-gray-400 dark:text-gray-600 select-none mr-4 text-xs">${idx + 1}</span><pre class="flex-1 text-sm text-slate-800 dark:text-slate-200 font-mono whitespace-pre-wrap break-words">${escapeHtml(line)}</pre></div>`)
      .join('')
  }, [content, language])
