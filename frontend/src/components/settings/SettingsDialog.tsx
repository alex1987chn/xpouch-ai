...
export function SettingsDialog({ isOpen, onClose }: SettingsDialogProps) {
  ...
  return createPortal(
    ...
    >
      <div
        className="relative bg-surface-card border-2 border-border-default shadow-[rgb(var(--shadow-color))_8px_8px_0_0] w-[600px] max-w-[90vw] max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b-2 border-border-default shrink-0">
          ...
          <button
            onClick={handleClose}
            className="w-6 h-6 flex items-center justify-center border border-border-default hover:bg-[rgb(var(--accent-hover))] transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    selectedModelId === model.id
                      ? 'border-[var(--accent-hover)] bg-[rgb(var(--accent-hover))]/10'
                      : 'border-border-default hover:border-[var(--text-secondary)]'
                  )}
                >
                  ...
                  {selectedModelId === model.id && (
                    <div className="w-5 h-5 border-2 border-[var(--accent-hover)] bg-[rgb(var(--accent-hover))] flex items-center justify-center">
        ...
        <div className="flex gap-0 border-t-2 border-border-default shrink-0">
          ...
          <button
            onClick={handleSave}
            className="flex-1 py-3 bg-[rgb(var(--accent-hover))] text-black font-mono text-sm font-bold uppercase hover:brightness-95 transition-colors"
          >
