...
const STYLES = {
  default: {
    // 卡片基础样式
    bg: 'bg-surface-card',
    border: 'border border-border-default/80',
    shadow: 'shadow-soft',
    // 悬停效果
  ...
  defaultAgent: {
    ...
    shadow: 'shadow-glow-accent/10',
    // 悬停效果
    hoverBg: 'hover:bg-surface-card',
    hoverBorder: 'hover:border-accent/50',
    hoverShadow: 'hover:shadow-glow-accent/20',
    hoverTransform: 'hover:-translate-y-0.5',
    // 图标容器
    iconBg: 'bg-gradient-to-br from-accent/10 to-accent/5',
    iconColor: 'text-accent',
    iconHoverColor: 'text-accent-hover',
  },
} as const
...
function AgentCard({ agent, isSelected, onClick, onDelete, onCreateAgent }: AgentCardProps) {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          'bg-gradient-to-br from-surface-page/80 to-surface-elevated/50',
          'border border-dashed border-border-default/70',
      ...
      >
        <div className="flex flex-col items-center gap-1.5 sm:gap-2">
          <div className={cn(
            ...
            'bg-gradient-to-br from-surface-card to-surface-page',
            'shadow-sm border border-border-default/50',
            'flex items-center justify-center',
            'transition-all duration-300 ease-out',
            'group-hover:scale-110 group-hover:shadow-lg group-hover:border-accent/50'
          )}>
  ...
  return (
    ...
    >
      ...
      <Card className={cn(
        'relative z-10 bg-surface-card/50 border-0 shadow-inner rounded-xl h-full transition-all duration-300 backdrop-blur-sm'
      )}>
              ...
              {!isDefaultAgent && (
                <button
                  ...
                  className={cn(
                    ...
                    'text-content-muted hover:text-content-inverted',
                    'opacity-0 group-hover:opacity-100',
                    'bg-gradient-to-r from-transparent via-transparent to-transparent',
                    'hover:bg-gradient-to-r from-accent-destructive/10 to-accent-destructive/20',
