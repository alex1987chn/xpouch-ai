...
- 被遗弃的计划不会永久挂着：审批等待超过 24 小时自动取消（`APPROVAL_TIMEOUT_HOURS` 可配），会话内留可见说明
- **用户管理与审计日志**（管理员）：脱敏用户列表、角色编辑、密码重置；管理面变更与计划裁决全量留痕
...
- 按用户隔离的长期记忆（pgvector 向量检索）——相关记忆同时注入路由决策与简单模式回复
- 说"记住…"，记忆专家提炼可长期保存的第三人称事实（一行一条，写入幂等去重）
- 自然语言管理："删掉我关于XX的记忆" / "我记住了什么"——先预览匹配范围，再删除并出具清单
- 严格用户隔离：身份由服务端闭包注入、绝不作为模型参数——不存在跨用户读删
...
- **审计日志**：管理面变更（用户/专家/配额/并发上限）与计划裁决动作（批准/修订/终止）全量留痕，记录操作者、动作、对象与详情，支持搜索；**只记结构性事实，不记对话与计划内容**（治理留痕不是内容副本）
...
- 用户管理与审计日志（脱敏列表、角色编辑、密码重置、管理面留痕、计划裁决留痕）
