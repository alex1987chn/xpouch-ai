...
│   │   ├── store/                     # Zustand 状态管理
│   │   │   ├── chatStore.ts           # 对话状态
│   │   │   ├── taskStore.ts           # 任务状态（Map 存储 + Selectors）
│   │   │   ├── userStore.ts           # 用户状态
│   │   │   └── middleware/            # 自定义中间件
│   │   │       └── persist.ts         # 持久化中间件
│   │   ├── hooks/                     # 自定义 Hooks
│   │   │   ├── useChat.ts             # 聊天主逻辑（组合 useChatCore + useExpertHandler）
│   │   │   ├── useChatCore.ts         # 聊天核心（SSE 连接、消息处理）
│   │   │   ├── useTaskSelectors.ts    # Zustand Selectors（性能优化）
│   │   │   ├── useChatSelectors.ts    # ChatStore Selectors
│   │   │   ├── useSessionRestore.ts   # 会话恢复（页面刷新后恢复状态）
│   │   │   ├── useSwipeBack.ts        # 移动端滑动手势
│   │   │   └── chat/                  # 聊天相关 hooks
│   │   │       ├── useChatCore.ts     # SSE 流式连接管理
│   │   │       ├── useExpertHandler.ts # 专家事件处理（task 状态更新）
│   │   │       ├── useConversation.ts  # 会话管理（新建/切换）
│   │   │       └── useSessionRecovery.ts # 会话恢复逻辑
│   │   ├── utils/                     # 工具函数
│   │   │   └── export.ts              # Artifact 导出工具（Markdown/PDF）
│   │   ├── services/                  # API 服务层
│   │   │   ├── api.ts                 # API 客户端
│   │   │   └── chat.ts                # 聊天 API
│   │   ├── handlers/                  # 事件处理器
...
│   ├── routers/                       # API 路由模块
│   │   ├── chat.py                    # 聊天 API（核心 SSE 流式接口）
│   │   ├── agents.py                  # 智能体 API（直接调用专家）
│   │   ├── admin.py                   # 管理员 API（专家配置/用户管理）
│   │   ├── auth.py                    # 认证 API（登录/注册/Token）
│   │   ├── system.py                  # 系统 API（健康检查/配置）
│   │   └── memory.py                  # 记忆 API（向量检索）
│   ├── routers/                       # 路由模块
│   │   ├── chat.py                    # 聊天 API
│   │   ├── agents.py                  # 智能体 API
│   │   └── system.py                  # 系统 API
...
│   │   └── db.py                      # 数据库连接（HITL）
│   ├── event_types/                   # 事件类型定义
│   │   └── events.py                  # SSE 事件协议（v3.0）
│   ├── migrations/                    # 数据库迁移
│   │   ├── apply_all_migrations.sql   # 统一迁移脚本
│   │   ├── apply_migrations.py        # Python 迁移脚本（带日志）
│   │   ├── rollback_migration.py      # 迁移回滚脚本
│   │   ├── README.md                  # 迁移说明文档
│   │   └── archive/                   # 历史迁移脚本
│   ├── scripts/                       # 脚本目录
│   │   ├── init_db.py                # 数据库表初始化
│   │   ├── init_checkpoints.py       # LangGraph 检查点表初始化（HITL）
