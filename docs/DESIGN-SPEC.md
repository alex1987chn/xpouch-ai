...
- **嵌套同心圆**：内圆角 = 外圆角 − 内边距（卡片内嵌 chips/头像槽/徽章按此计算，勿随手同值）。
...
- 材质：丝绒玻璃（backdrop-blur）只进浮层（modal / dropdown / 命令面板），不进正文区——性能税。
...
- 空态：圆形图标底 + 主文案 + 副引导；列表空态用虚线卡。**空态三分口径**：
  ...
  **空态只告知状态、不放操作按钮**（用户知道入口在哪；侧栏空态放新建 CTA 曾被点名突兀）。
...
- 搜索住**标题行右槽**（w-64/w-72），禁全宽浮动条；placeholder 一律 `content-muted`
  （secondary 深得像已输入的内容）。
...
- 动效：fast 100ms / normal 200ms；禁用 >2px 位移的装饰动画（顶栏 logo 除外）；
  尊重 prefers-reduced-motion。动效一律 CSS 实现（缓动用 `linear()`）——**勿引回 framer-motion**
  （依赖已刻意移除，勿为弹簧动效复活它）。
- 审批（待裁决）行/弹窗图标用 `ClipboardList`，不用 `AlertTriangle`——感叹号会被读成告警；
  琥珀=待裁决的语义不变。
