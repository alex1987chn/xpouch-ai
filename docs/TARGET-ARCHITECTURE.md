...
- [x] **第 3 片 · 读取端切换 + 清理（已完成 2026-09-13，commit `5b5d04f`）**：
  ...
  - **仍未覆盖的边界（明确记录，非缺陷）**：
    1. **首轮流（`handle_langgraph_stream`，即批准之前的规划阶段）压根不经过 hub** → 没有 `id:` 行、前端 `lastSeq` 恒为 0、不会触发续传。
       - **2026-09-13 复核结论：光把它接到 hub 并不够。** 首轮流的图循环跑在 SSE 生成器里（恢复流则跑在分离的 producer 任务里），客户端一断连 Starlette 就 `aclose()` 这个生成器 → 图循环被放弃 → **这个 run 当场死亡**。所以即使补上 seq、客户端再 resume，也没有任何东西可跟随（410 → 轮询 → 等清理循环把它标超时）。
       - **正确做法是一个小批的三件事**：① 把首轮流的图执行改成与恢复流相同的「分离 producer + 队列」结构（断连不再杀任务）；② 首轮流也走统一发布出口（有 seq、可重放）；③ 读端复用现成能力（缺口补放 + 实时跟随）——于是断连期间错过的 `human.interrupt` 能被重放，**审批卡自己就回来了**（目前断连后卡片只能靠刷新或去任务控制页拿）。
       - 在三件事做完之前，当日行为已比之前好：断流走「后台执行中」提示 + 轮询（不再只有一句「请重试」），但被中断的 run 仍会在超时后被标成失败。
...
### 批次 D 补 · 断流后的前端接管（2026-09-13，commit `ac75a91`）
