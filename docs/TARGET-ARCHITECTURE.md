...
- [x] **第 3 片 · 读取端切换 + 清理（已完成 2026-09-13，commit `5b5d04f`）**：
  ...
  - **仍未覆盖的边界（明确记录，非缺陷）**：
    ...
    3. **刷新页面（而非断线重连）后审批卡仍不出现**：断线重连走的是 resume 重放（本片已修），而 F5 后走的是 `useSessionRestore`——它只恢复消息与 run 状态，不会重建待审计划。补救路径现成：`/runs/{id}/plan`（HITL v4 修订轮询加的那个端点）能取回计划，在 restore 里对 `waiting_for_approval` 的 run 调一次 `setPendingPlan` 即可（前端小改，未做）。
