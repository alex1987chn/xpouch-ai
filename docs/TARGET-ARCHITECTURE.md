...
### 收尾四条（2026-09-13 晚，用户点名「全做」）

按可见价值排序做掉，每条一个 commit、各自实机验证过：

1. **无效会话不再静默空态 + 未知路径回首页留痕**（`7fb88c2`）。`useSessionRestore` 此前把
   「会话不存在（404）」与「新会话还没建好」当成同一件事——两者都是一句 debug 日志然后停在
   空页面，**看起来与「打开首页」一模一样且不会重试**；路由表里的未知路径又是裸
   `<Navigate to="/" replace />`，不留痕迹，于是这类现象完全无法归因。现在：能走到 404 的只可能是
   「会话指向不存在」（新会话那条路 `enabled=false` 会提前返回，根本到不了），故给显式状态
   `isMissingSession` + 重试/回工作台，并**先静默重试一次**（1.2s，躲开首条消息刚落库的可见性差）；
   未知路径改走 `NotFoundRedirect`（仍回工作台，但记一条 warning）。新增 2 条 hook 测试。
2. **会话状态标签对账**（`b11bfe8`）。chip 读 `latest_run.status`，而列表查询是
   `staleTime 5min + refetchOnWindowFocus:false`——状态在别处变（另一个标签页/任务控制页/运维容器）
   时没有任何对账时机。开 `refetchOnWindowFocus`，并在打开会话（刚拿到该线程真相）时失效列表一次。
   实机：待审批会话的 run 绕过应用在库里改成 cancelled → 切走再回来，chip 立刻从「待审核」变「1 分钟」。
3. **专家显示名读服务端名册**（`d01068b`）。权威名在 `systemexpert.name`（管理员可改），
   前端却只有两份会漂移的副本（静态词条 + 只含自定义专家的 `/agents`）。新增**任何登录用户可读**的
   `GET /api/experts/catalog`（**只回 key+name**，契约测试锁住不含 system_prompt/model/温度）；
   前端解析顺序抽成纯函数 `resolveExpertLabel`：名册 → 自定义列表 → 静态词条 → 原 type。
   实机：把库里 `aggregator` 的 name 改成「汇总官·改」→ 面板组头随之变，改回即恢复。
4. **SubTask 时刻字段落库**（`6095faa`）。查实 `completed_at`/`duration_ms` 一直有写，只有
   `started_at` **恒为 NULL**（执行期无处写；且收尾收集结果时会用图状态里的空值把它**覆盖成 NULL**，
   这是第二处、隐藏的）。修法=任务首次进入时后台写一次 `update_subtask_status(RUNNING)`
   （复用原本无人调用的函数），并把收尾回填改成 `or` 语义。实机 3 任务计划三行时刻齐全、
   时间链连贯（14:42:04→23→38→51，串行）。**运维含义：现在一条 SQL 能答"哪个任务慢"。**
