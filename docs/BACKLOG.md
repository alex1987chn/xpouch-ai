...
- [ ] **生产部署收尾（原 v3.5.4 验证，现应连 09-22/23 批次一起发下一个版本）**：确认 f6ae155（deploy.sh 改为 Compose v2 插件优先 + 版本闸门 ≥2.17）修复后重跑 deploy.sh 成功。前置检查：`.env` 空串 key（4851b97 起空串=拒绝启动）；**迁移 20260918_000100 会删 customagent 表与 custom 线程（不可恢复，备份兜底）**；20260916_000200 枚举 USING 转换=锁表操作，低峰执行；若设过日配额，b98478a 已修，部署后聊天即恢复。**新批次注意**：迁移 20260923_000200 会把内置专家（search/router/aggregator/memorize_expert）教材覆盖为代码种子版——若生产管理台人工调优过这几家提示词，先导出备份；另会补种 router/aggregator（本就该在）并把内置标记归位（is_dynamic=true 的用户自建专家不碰）。
